//
// Created by lucai on 10/17/2022.
//
...
void Camera::Inputs(GLFWwindow *window) {
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
        ...
        float rotX = sensitivity * (float) (mouseY - ((float) height / 2)) / (float) height;
        float rotY = sensitivity * (float) (mouseX - ((float) width / 2)) / (float) width;
